	...
	for _, part := range parts {
		...
		case "tool_use":
			toolCalls = append(toolCalls, ToolCallRequest{
				ID:   strings.TrimSpace(kitutil.Interface2String(part["id"])),
				Type: "function",
				Function: FunctionRequest{
					Name:      strings.TrimSpace(kitutil.Interface2String(part["name"])),
		...
		case "tool_result":
			toolMessage := Message{
				...
				ToolCallId: firstNonEmptyString(
					kitutil.Interface2String(part["tool_use_id"]),
					kitutil.Interface2String(part["tool_call_id"]),
					kitutil.Interface2String(part["id"]),
				),
			}
			if name := strings.TrimSpace(kitutil.Interface2String(part["name"])); name != "" {
		...
		case "text", "input_text", "output_text":
			...
			if cacheControl, exists := part["cache_control"]; exists && cacheControl != nil {
				if raw, err := kitutil.Marshal(cacheControl); err == nil {
		...
		default:
			if raw, err := kitutil.Marshal(part); err == nil {
				var media MediaContent
				if err := kitutil.Unmarshal(raw, &media); err == nil && media.Type != "" {
					mediaParts = append(mediaParts, media)
				}
			}
		}
...
func anthropicToolInputToArguments(input any) string {
	...
	encoded, err := kitutil.Marshal(input)
	if err != nil {
		return "{}"
	}
...
func anthropicToolResultContentToString(content any) string {
	...
	case []any:
		...
		for _, item := range typed {
			...
			if strings.TrimSpace(kitutil.Interface2String(part["type"])) == "text" {
	...
	if err != nil {
		return kitutil.Interface2String(content)
	}
	return string(encoded)
}
...
func (t *ToolCallRequest) UnmarshalJSON(data []byte) error {
	...
	if err := kitutil.Unmarshal(data, &canonical); err != nil {
		return err
	}
	*t = ToolCallRequest(canonical)
	...
	if err := kitutil.Unmarshal(data, &topLevelFunction); err != nil {
		return err
	}
